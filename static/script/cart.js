...
fetch('http://' + location.host + '/api/my/cart/', {
    ...
    .then(data => {
        ...
        data.products.forEach(item => {
            ...
            amount.onchange = () => onSelectionChange(item.article, amount.valueAsNumber);
...
function deleteFromCart(article) {
    ...
    fetch('http://' + location.host + '/api/my/cart/' + article + '/', {
        method: "DELETE",
        headers: {
...
function deleteProduct(id) {
    total -= productPrice[productArticles.indexOf(Number(id))] * amountInputs[productArticles.indexOf(Number(id))] / 100;
    deleteFromCart(id)
    updatePrice()
}
...
function onInitializate() {
    ...
    decrementButtons.forEach((button, index) => {
        button.addEventListener('click', () => {
            if (inputs[index].valueAsNumber > 1) {
                inputs[index].valueAsNumber--;
                amountInputs[index]--;
                scaleAmount(productArticles[index], inputs[index].valueAsNumber);
                total -= productPrice[index] / 100;
                ...
                total -= productPrice[index] / 100;
                console.log(total)
                deleteFromCart(productArticles[index]);
                updatePrice()
            }
        });
    });
...
function onSelectionChange(article, amount) {
    amountInputs[productArticles.indexOf(Number(article))] = amount;
    total = amount * productPrice[productArticles.indexOf(Number(article))] / 100;
    updatePrice();
    scaleAmount(productArticles[productArticles.indexOf(Number(article))], amount);
}
